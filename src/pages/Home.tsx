import React from 'react';
import { IonContent, IonHeader, IonPage, IonTitle, IonToolbar, IonGrid, IonRow, IonCol, IonIcon } from '@ionic/react';
import { people, cash } from 'ionicons/icons'
import Curve from '../components/Curve';
import './Home.css';

const Home: React.FC = () => {
  const heartLevel = 0.25;
  const feelLevel = 0.5;
  const businessLevel = 0.7;
  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          <IonTitle>Swipe to #FlattenTheCurve</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
        <IonGrid>
          <IonRow className="resource-container">
            <IonCol>
              <svg xmlns='http://www.w3.org/2000/svg' width='7vw' height='7vh' viewBox='0 0 512 512'>
                <title>Heart</title>
                <defs>
                    <linearGradient id="progress" x1="0" y1="1" x2="0" y2="0">
                    <stop id="stop1" offset={heartLevel} stop-color="red"/>
                    <stop id="stop2" offset={heartLevel} stop-color="black"/>
                    </linearGradient>
                </defs>
                <path 
                  d='M352.92,80C288,80,256,144,256,144s-32-64-96.92-64C106.32,80,64.54,124.14,64,176.81c-1.1,109.33,86.73,187.08,183,252.42a16,16,0,0,0,18,0c96.26-65.34,184.09-143.09,183-252.42C447.46,124.14,405.68,80,352.92,80Z' 
                  fill="url(#progress)" 
                  stroke="#FAA" 
                  stroke-width="20px"
                />
              </svg>
            </IonCol>
            <IonCol>
              <svg xmlns='http://www.w3.org/2000/svg' width='7vw' height='7vh' viewBox='0 0 512 512'>
                <title>People</title>
                <defs>
                    <linearGradient id="progress" x1="0" y1="1" x2="0" y2="0">
                    <stop id="stop1" offset={feelLevel} stop-color="red"/>
                    <stop id="stop2" offset={feelLevel} stop-color="black"/>
                    </linearGradient>
                </defs>
                <path 
                  d='M402,168c-2.93,40.67-33.1,72-66,72s-63.12-31.32-66-72c-3-42.31,26.37-72,66-72S405,126.46,402,168Z M336,304c-65.17,0-127.84,32.37-143.54,95.41-2.08,8.34,3.15,16.59,11.72,16.59H467.83c8.57,0,13.77-8.25,11.72-16.59C463.85,335.36,401.18,304,336,304Z M200,185.94C197.66,218.42,173.28,244,147,244S96.3,218.43,94,185.94C91.61,152.15,115.34,128,147,128S202.39,152.77,200,185.94Z M206,306c-18.05-8.27-37.93-11.45-59-11.45-52,0-102.1,25.85-114.65,76.2C30.7,377.41,34.88,384,41.72,384H154' 
                  fill="url(#progress)" 
                  stroke="#FAA" 
                  stroke-width="20px"
                />
              </svg>
            </IonCol>
            <IonCol>
              <svg xmlns='http://www.w3.org/2000/svg' width='7vw' height='7vh' viewBox='0 0 512 512'>
                <title>Heart</title>
                <defs>
                    <linearGradient id="progress" x1="0" y1="1" x2="0" y2="0">
                    <stop id="stop1" offset={businessLevel} stop-color="red"/>
                    <stop id="stop2" offset={businessLevel} stop-color="black"/>
                    </linearGradient>
                </defs>
                <path 
                  d='M448,256c0-106-86-192-192-192S64,150,64,256s86,192,192,192S448,362,448,256Z' 
                  fill="url(#progress)" 
                  stroke="#FAA" 
                  stroke-width="20px"
                />
              </svg>
            </IonCol>
          </IonRow>
          <IonRow className="curve-container">
            <Curve />
          </IonRow>
          <IonRow className="cardstack-container">
            <IonCol>Cardstack</IonCol>
          </IonRow>
        </IonGrid>
      </IonContent>
    </IonPage>
  );
};

export default Home;
